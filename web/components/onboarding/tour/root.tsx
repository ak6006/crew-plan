...
import PlaneWhiteLogo from "public/plane-logos/crew-lg-576.png";
...
const TOUR_STEPS: {
...
}[] = [
  {
    ...
    description:
      "The issue is the building block of the CREW Plan. Most concepts in CREW plan  are either associated with issues and their properties.",
    image: IssuesTour,
    nextStep: "cycles",
  },
  {
      ...
      {step === "welcome" ? (
        <div className="w-4/5 md:w-1/2 lg:w-2/5 h-3/4 bg-custom-background-100 rounded-[10px] overflow-hidden">
          <div className="h-full overflow-hidden">
            ...
            <div className="h-3/5 bg-black grid place-items-center">
              <Image src={PlaneWhiteLogo} alt="CREW PLAN Logo" />
            </div>
            <div className="h-2/5 overflow-y-auto p-6">
              <h3 className="font-semibold sm:text-xl">
                Welcome to CREW PLAN, {user?.first_name} {user?.last_name}
              </h3>
              <p className="text-custom-text-200 text-sm mt-3">
                We{"'"}re glad that you decided to try out crew plan. You can now manage your projects
        ...
        <div className="relative w-4/5 md:w-1/2 lg:w-3/5 h-3/5 sm:h-3/4 bg-custom-background-100 rounded-[10px] grid grid-cols-10 overflow-hidden">
          ...
          <div className="col-span-10 lg:col-span-7 h-full overflow-hidden">
            <div
              className={`flex items-end h-1/2 sm:h-3/5 overflow-hidden bg-custom-background-100 ${
